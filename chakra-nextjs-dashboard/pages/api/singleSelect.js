...
export default async function handler(req, res) {
  try {
    ...
    const data = await db.all('SELECT sd.create_date, sd.watts as solar_watts, sd.surface_temperature, sd.ambient_temperature, hd.watts as hydro_watts, hd.flowrate, wd.watts as wind_watts FROM solar_data sd JOIN hydro_data hd ON sd.create_date = hd.create_date JOIN wind_data wd ON sd.create_date = wd.create_date ORDER BY sd.create_date DESC Limit 1');
